  ...
  return (
    <div className="flex flex-col justify-between bg-slate-900/60 backdrop-blur-md border border-white/10 rounded-2xl p-5 md:p-6 shadow-xl relative overflow-hidden">
      ...
      <div>
          ...
          <div className="flex items-center gap-2">
            ...
            {onRefresh && (
              <button
                onClick={onRefresh}
                disabled={loading}
                className="text-[11px] px-2.5 py-1 text-gray-400 hover:text-white bg-slate-800/60 hover:bg-slate-700/60 border border-white/5 rounded-md transition disabled:opacity-50"
              >
                {loading ? '...' : 'Refresh'}
              </button>
            )}
        ...
        <div className="grid grid-cols-3 gap-3 py-3 mb-3 bg-slate-800/30 rounded-xl px-4 md:px-5 border border-white/5">
        ...
        {/* Advance / Decline Bar */}
        <div className="relative h-3.5 md:h-4 w-full rounded-full overflow-hidden flex bg-slate-800/80 shadow-inner my-3">
          <motion.div
      ...
      <div className="pt-3 mt-3 border-t border-white/5">
        <div className="flex items-center gap-1.5 text-xs font-medium text-gray-400 mb-2.5">
          <FontAwesomeIcon icon={faLayerGroup} className="w-3.5 h-3.5 text-indigo-400" />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
          {tierList.map((tier) => {
            ...
            return (
              <div
                key={tier.label}
                className="bg-slate-800/40 border border-white/5 rounded-xl p-3 md:p-3.5 flex flex-col justify-between"
              >
                <div className="flex justify-between items-center text-xs text-gray-400 mb-1.5">
                  <span className="font-semibold text-gray-200">{tier.label}</span>
                  <span className="font-mono text-gray-500">{tot}</span>
                </div>
                <div className="flex items-center justify-between text-xs md:text-sm font-mono font-bold mb-2">
                  <span className="text-emerald-400">{adv}↑</span>
                  <span className="text-xs text-gray-400 font-normal font-mono">{pct}%</span>
